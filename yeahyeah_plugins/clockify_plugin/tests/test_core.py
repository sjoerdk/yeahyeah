...
def some_projects():
    return [
        Project(obj_id=None, name="project1"),
...
@pytest.mark.parametrize(
    "cli_function, params", [(stop, None), (add, ["a message"]), (projects, None)]
)
def test_clockify_exceptions(
    mock_api_session, mock_clockify_runner, cli_function, params
):
    """Clockify API Exceptions should be caught and displayed, instead of raised"""
    for func in [
        mock_api_session.stop_timer,
    ...
    result = mock_clockify_runner.invoke(
        cli_function, args=params, catch_exceptions=False
    )
...
def test_add_message_parsing(
    mock_api_session, mock_clockify_runner, some_projects, args, expected_message
):
    ...
    assert result.exit_code == 0
    assert (
        mock_api_session.add_time_entry.call_args[1]["description"] == expected_message
    )
