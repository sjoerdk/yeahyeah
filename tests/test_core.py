...
def a_yeahyeah_instance(tmpdir):
    """A YeahYeah instance with tmp configuration path and default yeahyeah_plugins"""
...
def test_command_admin_status(a_yeahyeah_instance_with_plugins, mock_cli_runner):
    ...
    result = mock_cli_runner.invoke(
        a_yeahyeah_instance_with_plugins.root_cli,
...
def test_command_admin_edit_plugins(
    a_yeahyeah_instance, mock_cli_runner, disable_click_launch
):
    """Test the yeahyeah admin command"""
    result = mock_cli_runner.invoke(
        a_yeahyeah_instance.root_cli, args="admin yeahyeah edit-plugins".split(" ")
    )
    assert result.exit_code == 0
...
def test_yeahyeah_settings_load_errors():
    ...
    with pytest.raises(YeahYeahPersistenceException):
        YeahYeahSettingsFile(
            path=RESOURCE_PATH / "yeahyeah_settings_corrupt.json"
        ).load_settings()
    with pytest.raises(FileNotFoundError):
        YeahYeahSettingsFile(path="non_existent_path").load_settings()
...
def test_plugin_load_errors(a_yeahyeah_instance):
    """Load all kinds of crap but not valid plugin. Helpful exceptions should
    happen
    """
