...
def fake_now_time(monkeypatch):
    monkeypatch.setattr('clockify_plugin.parameters.utcnow',
                        lambda: datetime(year=2010, month=1, day=1, hour=6, minute=0, second=1, microsecond=0))


@pytest.mark.parametrize("input, expected_output", [(None, None),
                                                    ('14:34', '14:34'),
                                                    ('03:12', '03:12'),
                                                    ('+1:02', '07:02'),
                                                    ('-2:15', '03:45'),
...
def test_time(fake_now_time, input, expected_output):
    ...
    assert to_time_string(TIME.convert(value=input, param=None, ctx=None)) == expected_output
